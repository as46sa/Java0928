	...
	$(function () {
		...
		$(".updateCount").change(function () {
			...
			if ( confirm("你确定要将 【" + name + "】商品修改数量为：" + count + " 吗?") ) {
				//发起请求。给服务器保存修改
				location .href =
						"http://localhost:8080/Shop_war/CartServlet?action=updateCount&count="+count+"&id="+id;
			} else {
...
<div id="main">
	<table>
		...
		<c:if test="${empty sessionScope.cart.items}">
			<%--如果购物车空的情况--%>
			<tr>
				<td colspan="5"><a href="pages/client/index.jsp">亲，当前购物车为空！快跟小伙伴们去浏览商品吧！！！</a>
				</td>
			</tr>
		</c:if>
		<c:if test="${not empty sessionScope.cart.items}">
			...
			<c:forEach items="${sessionScope.cart.items}" var="entry">
				<tr>
					...
					<td>
						<input class="updateCount" style="width: 80px;" goodsId="${entry.value.id}" type="text" value="${entry.value.count}">
					</td>
